				...
				<Row>
					<Col sm={12} className="hideFormGroup">
						...
						<Input name="classificationIdSeq" 
							...
							value={() => {this.state.selectedClassification.reduce(function(previousValue, currentValue) {
								return previousValue+= "," + currentValue.csCsiIdseq;
							});}}
							className="hidden"
							readOnly="readOnly"
						/>
					{this.renderClassificationPills()}
					<button type="button" onClick={() =>{this.openClassificationModal();}} className="btn-link align-left">SEARCH FOR CLASSIFICATION</button>
					</Col>
				</Row>
			);
		}
