...
public class FormV2ExcelDownloadMangerImpl implements FormV2ExcelDownloadManger {
	...
	public byte[] downloadExcel(String formIdSeq, HttpServletRequest request) throws IOException {
		...
		List<FormV2TransferObject> crfs = new ArrayList<FormV2TransferObject>();
		...
		try {
			...
			for (String id : ids) {
				...
				crfs.add((FormV2TransferObject) formBuilderService.getFormDetailsV2(id));
			}
		} catch (Exception exp) {
		...
		try {
			...
			for (FormV2TransferObject crf : crfs) {
