...
export default (
	<Route path="/" component={App}>
		...
		<Route  cart="CDE" onEnter={isLoggedIn}  cartPageStateModel={cartsService.cartPageStateModel} cartData="cdeCartCollection" path={ROUTES.CART.VIEW_CDE_CART_PAGE} getComponent={(location, cb) => {
			cartChannel.request(EVENTS.CARTS.SET_LAYOUT, {action: cartActions.VIEW_CDE_CART_PAGE});
			cb(null, CartLayout);
		}} />
		<Route cart="Form" onEnter={isLoggedIn} cartData="formCartCollection" cartPageStateModel={cartsService.cartPageStateModel}  path={ROUTES.CART.VIEW_FORM_CART_PAGE} getComponent={(location, cb) => {
			cartChannel.request(EVENTS.CARTS.SET_LAYOUT, {action: cartActions.VIEW_FORM_CART_PAGE});
			cb(null, CartLayout);
		}} />
		<Route cart="Module" onEnter={isLoggedIn} cartPageStateModel={cartsService.cartPageStateModel} cartData="moduleCartCollection" path={ROUTES.CART.VIEW_MODULE_CART_PAGE} getComponent={(location, cb) => {
